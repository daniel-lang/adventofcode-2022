...
uint16_t run(uint16_t position, uint16_t minute, bool first = false)
{
    uint16_t best = 0;
    ...
    for (uint16_t i = 0; i < neededValveCount; ++i)
    {
        ...
        auto distance = directions.at(position).at(i);

        if (minute - distance <= 0)
            continue;

        closedValves &= ~(1 << i);
        best = std::max(best, static_cast<uint16_t>(flow[i] * (minute - distance - 1) + run(i, minute - distance - 1)));
        closedValves |= (1 << i);
...
int main(int, char *argv[])
{
    ...
    unsigned int maxFlown = run(getValveIndex("AA"), 30, true);
